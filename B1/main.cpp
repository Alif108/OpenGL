...
void drawInclinedGrid()
{
	...
	if(drawgrid==1)
	{
		...
		glBegin(GL_LINES);
		{
            ...
            {
				//lines parallel to Y-axis
				glVertex3f(i*10, -90, height);
				glVertex3f(i*10,  90, height);

				height += tan(INCLINATION_ANGLE * pi/180) * 10;         /// del_y = tangent * del_x
			}
...
void moveWheelForward()
{
    ...
    if(translatedPoint.x < 80)
        ...
        translatedPoint.z += tan(INCLINATION_ANGLE * pi/180) * STEP_SIZE * cos(direction_angle * pi/180);         /// del_y = tangent * del_x
